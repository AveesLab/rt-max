...
for glayer in $(seq $layer_start $layer_end); do
    ...
    for ((rlayer = $last_rlayer; rlayer <= $layer_num; rlayer++)); do
		...
		file_path_="measure/${reclaiming_accel_type}/${model}-multithread/${num_thread}thread/${glayer}glayer/cpu-reclaiming_${formatted_rlayer}rlayer_003split.csv"
		if [[ -f "$file_path_" ]]; then
			...
			if (( $(echo "$recaliming_infer < $gpu_infer" | bc) == 1 )); then
				...
				./darknet detector cpu-reclaiming_rlayer ./cfg/${data_file}.data ./cfg/${model}.cfg ./weights/${model}.weights data/dog.jpg -num_thread $num_thread -glayer $glayer -rlayer $rlayer -num_exp 10
				sleep 1s
			else
			...
			sleep 1s
			echo "GRC -- glayer: $glayer, rlayer: $rlayer, num_thread: $num_thread"
			./darknet detector cpu-reclaiming_rlayer ./cfg/${data_file}.data ./cfg/${model}.cfg ./weights/${model}.weights data/dog.jpg -num_thread $num_thread -glayer $glayer -rlayer $rlayer -num_exp 10 -opt_core $num_thread
			sleep 1s
		fi
	    else
	        break
