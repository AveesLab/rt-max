...
static int coreIDOrder[MAXCORES] = {0, 4, 1, 5, 3, 2};
...
void data_parallel_nano(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    ...
    strcat(file_path, "data-parallel_nano/");
    strcat(file_path, model_name);
    strcat(file_path, "/");

    strcat(file_path, "data-parallel_nano_");
