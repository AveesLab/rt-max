...
static double start_time[MAXCORES] = {0,0,0,0,0,0};
...
static int write_acceleration_info_before_compensation() {
    ...
    strcat(file_path, "gpu-accel-nano/");
...
void gpu_accel_nano(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
