...
static void threadFunc(thread_data_t data)
{
    ...
    for (int s = 0; s < net.n; s++){
        ...
        int Gend = net.n;    // GPU 작업 종료 레이어 인덱스

        if (data.thread_id == 1) printf("GPU-Accel with %d worker threads (GPU layers: %d-%d)\n", num_thread, Gstart, Gend);
        ...
        if (data.thread_id == 1) {
            ...
            if (data.thread_id == 1) printf("write_logs_to_files (GPU layers: %d-%d) --> worker_log_count: %d, gpu_log_count: %d\n", Gstart, Gend, worker_log_count, gpu_log_count);
