...
void cpu_inference(network_state *state, network *net, layer *l, int index)
{
    state->index = index;
    l = &(net->layers[index]);
    l->do_reclaiming = 0;
...
void gpu_inference(network_state *state, network *net, layer *l, int index)
{
    state->index = index;
    l = &(net->layers[index]);
    ...
    // if (skipped_layers[index]){
    //     l->output = l->output_gpu;      
    // }
    state->input = l->output_gpu;
}

void reclaiming_inference(network_state *state, network *net, layer *l, int index)
{
    state->index = index;
    l = &(net->layers[index]);
    l->do_reclaiming = 1;
...
static void threadFunc(int arg)
{
    ...
    int object_detection = strstr(g_cfgfile, target_model);
    ...
    for (i = 0; i < num_exp; i++) {
		...
		//         usleep(R * (thread_id - 1) * 1000);
        //     	}
        //     	else {
		//         pthread_barrier_wait(&barrier);
		//         usleep(R * (thread_id - 1) * 1000);
        //     	}
        //     	start_time[thread_id]=get_time_in_ms();
        //     }
        //     else{
		// start_time[thread_id] +=  R * num_thread;
		// remaining_time = start_time[thread_id] - get_time_in_ms();
