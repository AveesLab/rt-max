...
static double e_skip_layer[1000];
...
static int write_result(char *file_path) 
{
    ...
    for(i = 0; i < num_exp * optimal_core - startIdx; i++)
    {
        ...
        fprintf(fp, "%0.0f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f, %0.0f, %0.2f\n",  
...
static void threadFunc(thread_data_t data)
{
    ...
    for (i = 0; i < num_exp; i++) {
        ...
        // printf("start gpu : %0.2f \n", start_gpu_infer[count]);
#endif
        e_skip_layer[count] = 0;
        ...
        for(j = 0; j < gLayer; ++j){
            ...
            if (skipped_layers[j] == 1){
                // printf("skip layer : %d,  \n", j);
                double start_copy = get_time_in_ms();
                cuda_pull_array(l.output_gpu, l.output, l.outputs * l.batch);
                e_skip_layer[count] += (get_time_in_ms() - start_copy);
                // printf("start copy layer : %0.2f \n", start_copy);
                // printf("e_skip_layer : %0.2f \n", e_skip_layer[count]);
            }
    ...
    float hier_thresh, int dont_show, int theoretical_exp, int theo_thread, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    ...
    else strcat(file_path, "gpu-accel_gpu_copytime/");
