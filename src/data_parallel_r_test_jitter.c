    ...
    for (i = 0; i < num_exp; i++) {
        ...
        if (!data.isTest) {
            if (i == 5) {
                ...
                if (r_time != 0){
                    remaining_time = (double)r_time * (data.thread_id - 1);
    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    ...
    printf("\n\nData-parallel -- R Test with Jitter Compensation\n");
    printf("\n::TEST:: Data-parallel with %d threads with R (%d ms)\n", num_thread, r_time);
    ...
    strcat(file_path, "data-parallel_r_test_jitter/");
    ...
    char r_status[20];
    sprintf(r_status, "%dms_", r_time);
    strcat(file_path, r_status);
