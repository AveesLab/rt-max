...
reclaiming_accel_type="cpu-reclaiming-CRG"
...
for glayer in $(seq $layer_end -1 $layer_start); do
    if [ $(($glayer - 1)) -lt $last_rlayer ]; then
	last_rlayer=$(($glayer - 1))
    fi
    for ((rlayer = glayer - 1; rlayer >= 0; rlayer--)); do
		...
		if [[ -f "$file_path_" ]]; then
			...
			if (( $(echo "$recaliming_infer < $gpu_infer" | bc) == 1 )); then
				sleep 1s
				echo "CRG -- glayer: $glayer, rlayer: $rlayer, num_thread: $num_thread"
				./darknet detector ${reclaiming_accel_type} ./cfg/${data_file}.data ./cfg/${model}.cfg ./weights/${model}.weights data/dog.jpg -num_thread $num_thread -glayer $glayer -rlayer $rlayer -num_exp 15 -opt_core $num_thread
				sleep 1s
			else
